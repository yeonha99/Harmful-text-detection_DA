...
### tensorboard로 acc, loss에 대한 결과를 확인해보기 위한 코드 변경 
            ...
            if (step + 1) % args.pre_log_step == 0:
                print("Epoch [%.2d/%.2d] Step [%.3d/%.3d]: cls_loss=%.4f"
                      % (epoch + 1,
                         ...
                         len(data_loader),
                         cls_loss.item()))
             #   writer.add_scalar('cls_loss', cls_loss.item(), step)        
    ...
    for epoch in range(args.num_epochs):
        ...
        for step, ((reviews_src, src_mask, _), (reviews_tgt, tgt_mask, _)) in data_zip:
            ...
            if (step + 1) % args.log_step == 0:
               ...
               # writer.add_scalar('gen_loss', gen_loss.item(), step)
               # writer.add_scalar('dis_loss', dis_loss.item(), step)
               # writer.add_scalar('kd_loss', kd_loss.item(), step) 
